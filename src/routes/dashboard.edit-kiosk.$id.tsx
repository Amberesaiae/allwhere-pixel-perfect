...
export const Route = createFileRoute("/dashboard/edit-kiosk/$id")({
  head: () => ({
    meta: [
      { title: "Edit Kiosk | BlueKiosk" },
      { name: "description", content: "Edit your kiosk on BlueKiosk." },
    ],
  }),
  component: EditKioskPage,
});

type Category = Tables<"categories">;

function EditKioskPage() {
  const { id: kioskId } = Route.useParams();
  ...
  const fetchData = async () => {
    const [catRes, kioskRes] = await Promise.all([
      supabase.from("categories").select("*").order("name"),
      supabase.from("kiosks").select("*").eq("id", kioskId).single(),
    ]);

    setCategories(catRes.data || []);

    if (!kioskRes.data || kioskRes.data.owner_id !== user?.id) {
      navigate({ to: "/dashboard" });
      return;
    }

    const k = kioskRes.data;
    setName(k.name);
    setDescription(k.description || "");
    setCategoryId(k.category_id || "");
    setRegion(k.region || "");
    setCity(k.city || "");
    setPhone(k.phone || "");
    setExistingCoverUrl(k.cover_image_url);
    setLoading(false);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let coverUrl = removeCover ? null : existingCoverUrl;
  ...
  const displayCover = coverPreview || (removeCover ? null : existingCoverUrl);
    ...
    <>
      ...
      <main className="min-h-screen bg-bk-cream py-8">
        <div className="mx-auto max-w-[640px] px-6">
          <h1 className="text-[28px] font-bold text-bk-dark mb-2">Edit Kiosk</h1>
          <p className="text-[14px] text-bk-muted mb-8">Update your kiosk details</p>
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-bk-beige p-8 space-y-6">
            ...
            <div className="flex gap-3">
              <button
                type="submit"
                disabled={saving}
                className="flex-1 bg-bk-yellow text-bk-dark font-semibold text-[15px] py-3 rounded-full hover:bg-bk-yellow-hover transition disabled:opacity-50 flex items-center justify-center gap-2"
              >
              ...
              <button
                ...
                className="px-6 py-3 rounded-full border-2 border-bk-beige text-bk-dark font-semibold text-[15px] hover:bg-bk-beige transition"
              >
                Cancel
              </button>
            </div>
