...
import { Monitor, Shirt, UtensilsCrossed, Sparkles, Home, Car, BookOpen, Wrench, LayoutGrid } from "lucide-react";
        ...
        <div className="flex items-center gap-1 overflow-x-auto scrollbar-hide py-2 -mx-1 px-1">
          {categories.map((cat) => (
            <Link
              key={cat.slug || "all"}
              to="/discover"
              search={cat.slug ? { category: cat.slug, tab: "listings" } : { tab: "listings", category: "" }}
